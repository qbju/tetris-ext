...
int _start(void) {
    int c, i, length;
    if (state == 0) { out[0] = 0; puts("CAT FILE> "); state = 1; }
    if (state == 1) {
        while ((c = getchar()) >= 0) {
            if (c == 8 || c == 127) { if (name_len > 0) name_len--; }
            else if (c == '\n') { request(1); state = 2; return SYSCALL; }
            else if (name_len < 15) name[name_len++] = (char)c;
        }
        return WAIT;
    }
    if (state == 2) {
        if (call[1] != 0) { puts("\nNOT FOUND"); return EXIT; }
        puts("\n");
        length = call[4] | (call[5] << 8);
        for (i = 0; i < length && out[0] < 250; i++) {
            unsigned int n = out[0]; out[1 + n] = call[32 + i]; out[0] = (u8)(n + 1);
        }
        return EXIT;
    }
    return EXIT;
