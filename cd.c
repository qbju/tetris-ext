...
int _start(void){int c;if(state==0){out[0]=0;puts("CD> ");state=1;}if(state==1){while((c=getchar())>=0){if(c==8||c==127){if(name_len>0)name_len--;}else if(c==10){request(5);state=2;return SYSCALL;}else if(name_len<15)name[name_len++]=(char)c;}return WAIT;}if(state==2){result();return EXIT;}return EXIT;}